...
class CircularLinkedList:
    ...
    def print_list(self, starting_point=None):
        ...
        nodes = []
        for node in traverse(self, starting_point):
...
def part_two(filename: str) -> int:
    ...
    for round in range(10):
        mix_numbers(nodes_ordered, cll, round+1)
        # node = cll.head
        # while node.data != 0:
        #     node = node.next
        # cll.print_list(node)
...
def mix_numbers(nodes_ordered: list[Node], cll: CircularLinkedList, round=1):
    write_count = 0
    for node in nodes_ordered:
        ...
        if round == 2 and write_count < 10:
            mnode = cll.head
            while mnode.data != 0:
                mnode = mnode.next
            # cll.print_list(mnode)
            write_count += 1
            print(node.data, steps)
